
// File: C:\CodePrj\Gt2\Game\src\jon\shadow.c

/*
 * ModName: .\Debug\shadow.obj
 * (000004) Start search for segment 0x1 at symbol 0x8C(000010) S_OBJNAME: Signature: 00000000, C:\CodePrj\Gt2\Game\Debug\shadow.obj
 * 
 * (000040) S_COMPILE:
 *          Language: C
 *          Target processor: Pentium
 *          Floating-point precision: 0
 *          Floating-point package: hardware
 *          Ambient data: NEAR
 *          Ambient code: NEAR
 *          PCode present: 0
 *          Compiler Version: Microsoft (R) 32-bit C/C++ Optimizing Compiler Version 12.00.8447.0
 * 
 * (00008C) S_GPROC32: [0001:0002CF30], Cb: 0000060F, Type:             0x1A6A, DrawShadow
 *          Parent: 00000000, End: 00000350, Next: 00000354
 *          Debug start: 0000000A, Debug end: 0000060A
 *          Flags: Frame Ptr Present
 * 
 * (0000C0)  S_BPREL32: [00000008], Type:             0x1404, obj
 * (0000D0)  S_BPREL32: [0000000C], Type:      T_UCHAR(0020), penumbra
 * 
 * (0000E8)  S_BLOCK32: [0001:0002CF47], Cb: 00000202, (none)
 *           Parent: 0000008C, End: 000001E4
 * 
 * (000100)   S_BPREL32: [FFFFFF94], Type:             0x1A34, pos
 * (000110)   S_BPREL32: [FFFFFFA0], Type:             0x1A9D, ThisCar
 * (000124)   S_BPREL32: [FFFFFFA4], Type:     T_USHORT(0021), YR
 * (000134)   S_BPREL32: [FFFFFFA8], Type:      T_ULONG(0022), Pt
 * (000144)   S_BPREL32: [FFFFFFAC], Type:   T_32PULONG(0422), czone
 * (000158)   S_LDATA32: [0003:0000EEFC], Type:             0x1769, PtOrder
 * (000170)   S_BPREL32: [FFFFFFB0], Type:             0x1008, tmpPt
 * (000184)   S_BPREL32: [FFFFFFBC], Type:             0x1ABA, plan
 * (000198)   S_BPREL32: [FFFFFFFC], Type:      T_ULONG(0022), a
 * 
 * (0001A8)   S_BLOCK32: [0001:0002CFE5], Cb: 0000011A, (none)
 *            Parent: 000000E8, End: 000001E0
 * 
 * (0001C0)    S_BPREL32: [FFFFFF8C], Type:     T_REAL32(0040), z
 * (0001D0)    S_BPREL32: [FFFFFF90], Type:     T_REAL32(0040), x
 * 
 * (0001E0)   S_END
 * 
 * (0001E4)  S_END
 * 
 * (0001E8)  S_BLOCK32: [0001:0002D14E], Cb: 000003EC, (none)
 *           Parent: 0000008C, End: 0000034C
 * 
 * (000200)   S_BPREL32: [FFFFFE60], Type:             0x1BAC, TheShadow
 * (000218)   S_BPREL32: [FFFFFF60], Type:             0x1A9D, ThisCar
 * (00022C)   S_BPREL32: [FFFFFF64], Type:     T_USHORT(0021), YR
 * (00023C)   S_BPREL32: [FFFFFF68], Type:             0x103E, PtClipped
 * (000254)   S_BPREL32: [FFFFFF88], Type:      T_ULONG(0022), a
 * 
 * (000264)   S_BLOCK32: [0001:0002D19A], Cb: 0000033D, (none)
 *            Parent: 000001E8, End: 00000304
 * 
 * (00027C)    S_BPREL32: [FFFFFE34], Type:             0x1A34, Pos
 * (00028C)    S_BPREL32: [FFFFFE40], Type:             0x1008, StartP
 * (0002A0)    S_BPREL32: [FFFFFE4C], Type:   T_32PULONG(0422), czone
 * (0002B4)    S_BPREL32: [FFFFFE50], Type:             0x1008, Vec
 * (0002C4)    S_BPREL32: [FFFFFE5C], Type:     T_REAL32(0040), alpha
 * 
 * (0002D8)    S_BLOCK32: [0001:0002D2C8], Cb: 0000020F, (none)
 *             Parent: 00000264, End: 00000300
 * 
 * (0002F0)     S_BPREL32: [FFFFFE30], Type:             0x13B6, cf
 * 
 * (000300)    S_END
 * 
 * (000304)   S_END
 * 
 * (000308)   S_BLOCK32: [0001:0002D4DC], Cb: 0000005E, (none)
 *            Parent: 000001E8, End: 00000348
 * 
 * (000320)    S_LDATA32: [0003:0000EF0C], Type:             0x1B27, TriList
 * (000338)    S_BPREL32: [FFFFFDF0], Type:             0x1006, Mat
 * 
 * (000348)   S_END
 * 
 * (00034C)  S_END
 * 
 * (000350) S_END
 * 
 * (000354) S_LPROC32: [0001:0002D53F], Cb: 00000071, Type:             0x1AA5, meshFindCollisionZone
 *          Parent: 00000000, End: 00000410, Next: 00000414
 *          Debug start: 00000006, Debug end: 0000006D
 *          Flags: Frame Ptr Present
 * 
 * (000394)  S_BPREL32: [00000008], Type:     T_REAL32(0040), x
 * (0003A4)  S_BPREL32: [0000000C], Type:     T_REAL32(0040), z
 * (0003B4)  S_BPREL32: [FFFFFFF4], Type:      T_ULONG(0022), i
 * (0003C4)  S_BPREL32: [FFFFFFF8], Type:      T_ULONG(0022), iz
 * (0003D4)  S_BPREL32: [FFFFFFFC], Type:      T_ULONG(0022), ix
 * 
 * (0003E4)  S_BLOCK32: [0001:0002D593], Cb: 00000017, (none)
 *           Parent: 00000354, End: 0000040C
 * 
 * (0003FC)   S_BPREL32: [FFFFFFF0], Type:   T_32PULONG(0422), z
 * 
 * (00040C)  S_END
 * 
 * (000410) S_END
 * 
 * (000414) S_GPROC32: [0001:0002D5B0], Cb: 00000005, Type:             0x1BAE, shadeInitialize
 *          Parent: 00000000, End: 00000484, Next: 00000488
 *          Debug start: 00000003, Debug end: 00000003
 *          Flags: Frame Ptr Present
 * 
 * (00044C)  S_BPREL32: [00000008], Type:             0x13A1, vec
 * (00045C)  S_BPREL32: [0000000C], Type:     T_REAL32(0040), radNear
 * (000470)  S_BPREL32: [00000010], Type:     T_REAL32(0040), radFar
 * 
 * (000484) S_END
 * 
 * (000488) S_GPROC32: [0001:0002D5B5], Cb: 00000005, Type:             0x1001, shadeDestroy
 *          Parent: 00000000, End: 000004BC, Next: 0000075C
 *          Debug start: 00000003, Debug end: 00000003
 *          Flags: Frame Ptr Present
 * 
 * (0004BC) S_END
 * 
 * (0004C0) S_GDATA32: [0003:0121E600], Type:             0x1006, moxIdenMatrix
 * (0004DC) S_GDATA32: [0003:01255DF0], Type:             0x1ACA, MESH_Header
 * (0004F8) S_GDATA32: [0003:0097BED0], Type:     T_REAL32(0040), meshXZD
 * (000510) S_GDATA32: [0003:0097BED4], Type:     T_REAL32(0040), meshZZD
 * (000528) S_GDATA32: [0003:0097BEC8], Type:     T_REAL32(0040), meshXOff
 * (000540) S_GDATA32: [0003:0097BECC], Type:     T_REAL32(0040), meshZOff
 * (000558) S_GDATA32: [0003:00C9A980], Type:             0x1656, VidCtrl
 * (000570) S_GDATA32: [0003:0007B920], Type:  T_32PREAL32(0440), MATH_SinTable
 * (00058C) S_GDATA32: [0003:0007B924], Type:  T_32PREAL32(0440), MATH_CosTable
 * (0005A8) S_GDATA32: [0003:0091ED48], Type:             0x1B2B, RS_lpCurTex
 * (0005C4) S_GDATA32: [0003:00C97114], Type:       T_INT4(0074), RS_DumpPacket
 * (0005E0) S_GDATA32: [0003:00C970E0], Type:             0x1AD2, RS_TexChge
 * (0005FC) S_LDATA32: [0003:0000EDA0], Type:             0x1BB1, fCube
 * (000610) S_GDATA32: [0003:000757D8], Type:             0x1404, shadow
 * (000628) S_GDATA32: [0003:00C97110], Type:      T_ULONG(0022), RS_DPICalled
 * (000644) S_GDATA32: [0003:00C970C8], Type:      T_ULONG(0022), RS_DPIVCount
 * (000660) S_GDATA32: [0003:00C971BC], Type:      T_ULONG(0022), RS_DPIICount
 * (00067C) S_GDATA32: [0003:0000EE00], Type:             0x13BB, shadowtex
 * (000694) S_GDATA32: [0003:00C97100], Type:             0x1AD2, RS_TransformChge
 * (0006B4) S_LDATA32: [0003:000757DC], Type:             0x1A34, blankP
 * (0006CC) S_GDATA32: [0003:0006D568], Type:             0x1B2D, RS_PrimType
 * (0006E8) S_LDATA32: [0003:000757E8], Type:             0x1A35, blankO
 * (000700) S_GDATA32: [0003:0000EE20], Type:             0x1008, Sun
 * (000714) S_GDATA32: [0003:0097F150], Type:             0x1008, flVec
 * (000728) S_LDATA32: [0003:0000EE2C], Type:             0x1AB9, txFlare
 * (000740) S_LDATA32: [0003:0000EE6C], Type:             0x1BB2, ColorTable
 * 
 * (00075C) S_GPROC32: [0001:0002D5C0], Cb: 00000053, Type:             0x1BB4, RSTATES_SetTransform
 *          Parent: 00000000, End: 000007D4, Next: 000007D8
 *          Debug start: 00000003, Debug end: 00000051
 *          Flags: Frame Ptr Present
 * 
 * (000798)  S_BPREL32: [00000008], Type:       T_INT4(0074), dtstTransformStateType
 * (0007BC)  S_BPREL32: [0000000C], Type:             0x11DC, lpD3DMatrix
 * 
 * (0007D4) S_END
 * 
 * (0007D8) S_GPROC32: [0001:0002D620], Cb: 00000072, Type:             0x1BB6, RSTATES_SetTexture
 *          Parent: 00000000, End: 00000840, Next: 00000844
 *          Debug start: 00000003, Debug end: 00000070
 *          Flags: Frame Ptr Present
 * 
 * (000814)  S_BPREL32: [00000008], Type:      T_ULONG(0022), dwStage
 * (000828)  S_BPREL32: [0000000C], Type:             0x1308, lpTexture
 * 
 * (000840) S_END
 * 
 * (000844) S_GPROC32: [0001:0002D6A0], Cb: 00000096, Type:             0x1BB8, RSTATES_DrawIndexedPrimitive
 *          Parent: 00000000, End: 00000944, Next: 00000000
 *          Debug start: 00000003, Debug end: 00000094
 *          Flags: Frame Ptr Present
 * 
 * (000888)  S_BPREL32: [00000008], Type:       T_INT4(0074), d3dptPrimitiveType
 * (0008A8)  S_BPREL32: [0000000C], Type:      T_ULONG(0022), dwVertexTypeDesc
 * (0008C8)  S_BPREL32: [00000010], Type:    T_32PVOID(0403), lpvVertices
 * (0008E0)  S_BPREL32: [00000014], Type:      T_ULONG(0022), dwVertexCount
 * (0008FC)  S_BPREL32: [00000018], Type:  T_32PUSHORT(0421), lpwIndices
 * (000914)  S_BPREL32: [0000001C], Type:      T_ULONG(0022), dwIndexCount
 * (000930)  S_BPREL32: [00000020], Type:      T_ULONG(0022), dwFlags
 * 
 * (000944) S_END
 * 
 */

